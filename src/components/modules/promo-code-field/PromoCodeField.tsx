...
const PromoCodeField = ({ transfer, className }: any) => {
  ...
  const [img, setImg] = useState<
    null | "check-mark" | "crossed" | "rolling-loader-black"
  >(null);
  ...
  const handleCouponChange = async (value: string) => {
    ...
    dispatch({
      type: TRANSFER,
      payload: { ...transfer, promo: validatedPromo },
    });
  };
    ...
    <Field className={className}>
      <div className="coupon">
        <input
          ...
          name="promo-code"
          placeholder={promo ? promo.code : "Got a promo code? Get a discount"}
          onChange={(e) => handleCouponChange(e.target.value)}
        />
        {img && (
          <img
            className="check-mark"
            src={asset("icons", `${img}.svg`)}
            alt="check"
          />
        )}
        <div className="message-block">
          {promo && (
            <span className="green-txt">
              {" "}
              Value <b className="green-txt">: {getPromoValue(promo)} </b> -
              (Valid for {promo?.settings?.currenciesValid ?? "all"} currencies){" "}
            </span>
          )}
        </div>
      </div>
