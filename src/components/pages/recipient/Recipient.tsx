...
const RecipientCard = ({
  ...
  return (
    <div
      ...
      className={`recipient recipient-hoverable  ${
        selectedRecipient?.id === recipient.id && "selected-border"
      }`}
      onClick={() => {
    ...
    >
      <div
        className={`recipient-dropdown-container ${
          selectedRecipient?.id === recipient.id && "selected-icon"
        }`}
        onClick={() => setHoveredRecipientId(recipient.id)}
      >
        <img
          className="recipient-dropdown-btn"
          src={asset("icons", "three-dots.svg")}
          alt="dots"
        />
      </div>
      {hoveredRecipientId === recipient.id && (
        <div className="recipient-dropdown-option-container">
          <div className="icon-container">
            <img
              className="recipient-icon-delete"
              src={asset("icons", "bin.svg")}
              alt=""
            />
          </div>
          <div
      ...
      <div>
        ...
        {recipient.profile.transferMethod && (
          <small className="capitalize recipient-transfer-method">
            ({replaceUnderscores(recipient.profile.transferMethod)})
          </small>
        )}
        {recipient.profile.mobileMoneyProvider && (
          <small className="capitalize d-block recipient-transfer-method">
            {recipient.profile.mobileMoneyProvider}
          </small>
        )}
      </div>
...
const Recipient = () => {
  ...
  const checkPageAuthorization = (user: any) => {
    ...
    const mobileMoneyMax = getMax(
      transferMethod,
      toReceive?.countryCode,
      toSend?.countryCode
    );
    if (
      transferMethod === "mobile_money" &&
      Number(toReceive.total) > mobileMoneyMax
    ) {
      history.replace(paths.GET_QUOTE);
      toastAction({
        show: true,
        ...
        message: `The maximum transferable amount (inclusive of charges) for ${replaceUnderscores(
          transferMethod
        )} is ${mobileMoneyMax} ${toReceive.currency} for Mobile Money`,
      });
      return;
    }
    ...
    if (!userIsVerified(user)) {
      ...
      return history.replace(paths.VERIFICATION);
    }
    // } else {
    //   if (
    //     isUserFirstTransaction(user) &&
    //     Number(toSend.total) <
    //       maxTransfersUnverified[toSend?.currency?.toUpperCase()]
    //   ) {
  ...
  const handleRecipientClick = (recipient: any) => {
    if (recipient.profile.transferMethod !== transferMethod) {
      if (!editMode()) {
        return toastAction({
          ...
          type: "warning",
          timeout: 5000,
          message: `Select a recipient with ${transfer.transferMethod?.replace(
            "_",
            " "
          )} transfer method for this transaction`,
        });
      }
    }
    ...
    if (
      ...
      (recipient.profile.remittanceHandler ||
        remittanceHandlers.MANUAL_REMITTANCE_HANDLER)
    ) {
      return toastAction({
        show: true,
        type: "warning",
  ...
  const handleContinueEditMode = () => {
    ...
    updateTransferRecipient(
      () => history.push(paths.PAYMENT_METHOD + "?t=" + paramTransferId),
      paramTransferId
    );
  };
  ...
  return (
    <Body>
      ...
      <ProgressBar point={2} />
      <NewRecipientModal
        openModal={setOpenNRModal}
        modalOpen={openNRModal}
        selectRecipient={handleRecipientClick}
        recipientData={recipientDataForUpdate}
      />

      {transferDetailsModalOpen && (
        <div className="timeline-modal-container desktop-hide">
          <div
            className="overlay"
      ...
      <div className="page-content">
        ...
        <div className={openNRModal ? "mobile-hide" : ""}>
          <PageHeading
            heading="Recipient"
            subheading="Who are you sending money to?"
            back={paths.GET_QUOTE}
          />
          <div
            className="green-txt desktop-hide view-td is-link"
            onClick={() => setTransferDetailsModalOpen(true)}
          >
        ...
        <div className="box-container">
          <div className="right part">
            ...
            <div className="small-boxes">
              <div
                className="recipient plus"
                onClick={() => setOpenNRModal(true)}
              >
